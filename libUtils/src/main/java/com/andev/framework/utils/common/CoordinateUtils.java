package com.andev.framework.utils.common;
...
public final class CoordinateUtils {
    ...
    private static double transformLat(
        ...
        double ret = -100.0 + 2.0 * lng + 3.0 * lat + 0.2 * lat
    ...
    private static double transformLng(
        ...
        double ret = 300.0 + lng + 2.0 * lat + 0.1 * lng
                * lng + 0.1 * lng * lat + 0.1 * Math.sqrt(Math.abs(lng));
        ret += (20.0 * Math.sin(6.0 * lng * PI) + 20.0 * Math.sin(2.0 * lng * PI)) * 2.0 / 3.0;
    ...
    public static double getAngle(
        ...
        ret = 4 * Math.pow(Math.sin((radLat1 - radLat2) / 2), 2)
                - Math.pow(Math.sin((radLng1 - radLng2) / 2)
                * (Math.cos(radLat1) - Math.cos(radLat2)), 2);
        ret = Math.sqrt(ret);
        ret = ret / Math.sin(Math.abs(radLng1 - radLng2) / 2)
